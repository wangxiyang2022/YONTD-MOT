...
class Calibration(object):
    ''' Calibration matrices and utils
        3d_det XYZ in <label>.txt are in rect camera coord.
...
def compute_box_3d(obj, P):
    ''' Takes an object and a projection matrix (P) and projects the 3d_det
    ...
    # only draw 3d_det bounding box for objs in front of the camera
    if np.any(corners_3d[2,:]<0.1):
        corners_2d = None
        return corners_2d, np.transpose(corners_3d)
    
    # project the 3d_det bounding box into the image plane
    corners_2d = project_to_image(np.transpose(corners_3d), P);
    #print 'corners_2d: ', corners_2d
    return corners_2d, np.transpose(corners_3d)

def project_to_image(pts_3d, P):
    ''' Project 3d_det points to image plane.
...
def draw_projected_box3d(image, qs, color=(255,255,255), thickness=4):
    ''' Draw 3d_det bounding box in image
        qs: (8,2) array of vertices for the 3d_det box in following order:
            1 -------- 0
           /|         /|
          2 -------- 3 .
          | |        | |
