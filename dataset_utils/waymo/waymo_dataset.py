...
from detector.pcdet.pcdet.ops.roiaware_pool3d import roiaware_pool3d_utils
from detector.pcdet.pcdet.utils import box_utils, common_utils
from detector.pcdet.pcdet.datasets.dataset import DatasetTemplate
...
class WaymoDataset(DatasetTemplate):
    ...
    def get_infos(self, raw_data_path, save_path, num_workers=multiprocessing.cpu_count(), has_label=True, sampled_interval=1, update_info_only=False):
        ...
        print('---------------The waymo_casa sample interval is %d, total sequecnes is %d-----------------'
    ...
    def __getitem__(self, index):
        ...
        temp_images = info.get('image')
        for key in temp_images.keys():
            if isinstance(temp_images[key], str):
                temp_images[key] = str(self.data_path / temp_images[key])

        data_dict['images_path'] = temp_images
        data_dict['cam_calibration'] = info.get('cam_calibration')
        data_dict['pose'] = info.get('pose')
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        elif kwargs['eval_metric'] == 'waymo_casa':
...
def create_waymo_infos(dataset_cfg, class_names, data_path, save_path,
    ...
    test_filename = save_path / ('waymo_infoss_%s.pkl' % (test_split))
    ...
    print('----------------Waymo info test file is saved to %s----------------' % train_filename)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg_file', type=str, default="dataset_utils/waymo/waymo_dataset.yaml", help='specify the config of dataset')
    ...
    if args.func == 'create_waymo_infos':
        ...
        create_waymo_gt_database(
            dataset_cfg=dataset_cfg,
            class_names=['Vehicle', 'Pedestrian', 'Cyclist'],
            data_path=ROOT_DIR / 'data' / 'waymo_casa',
            save_path=ROOT_DIR / 'data' / 'waymo_casa',
            processed_data_tag=args.processed_data_tag,
