...
import pickle
import numpy as np
from detector.pcdet.pcdet.utils import common_utils
...
def process_single_sequence(sequence_file, save_path, sampled_interval, has_label=True, use_two_returns=True,
    ...
    cur_img_save_dir = []
    for i in range(1, 6):
        cur_img_save_dir.append(save_path / sequence_name / dataset_pb2.CameraName.Name.Name(i))
        cur_img_save_dir[-1].mkdir(parents=True, exist_ok=True)
    ...
    for cnt, data in enumerate(dataset):
        ...
        frame = dataset_pb2.Frame()  # 这只是一个类而已
        ...
        #     show_projected_lidar_labels(image, ax, frame)
        #
        # plt.show()
        ...
        # for index, image in enumerate(frame.images):
        #     ax = show_camera_image(image, [3, 3, index + 1])
        #     show_projected_camera_synced_boxes(image, ax, frame, draw_3d_box=False)
        #
        # plt.show()
        ...
        # save imgs
        for index, image in enumerate(frame.images):
            cam_type = dataset_pb2.CameraName.Name.Name(image.name)
            save_camera_image(image, cur_img_save_dir[image.name-1] / '{}.jpg'.format(str(cnt).zfill(4)))
            key_str = '{}_path'.format(cam_type)
            image_info.update({key_str: cur_img_save_dir[image.name-1] / '{}.jpg'.format(str(cnt).zfill(4))})
        ...
        for cc in frame.context.camera_calibrations:
            key_str = dataset_pb2.CameraName.Name.Name(cc.name)
            cam_calibration.update({key_str: cc})

        info["cam_calibration"] = cam_calibration
        ...
        if has_label:
            ...
            info['annos'] = annotations  # ego frame
...
import cv2
def save_camera_image(camera_image, save_path):
    """Save the given camera image."""
...
def show_projected_lidar_labels(camera_image, ax, frame):
...
def show_projected_camera_synced_boxes(camera_image, ax, frame, draw_3d_box=False):
    ...
    for label in frame.laser_labels:
        ...
        if (FILTER_AVAILABLE and not label.num_lidar_points_in_box):
