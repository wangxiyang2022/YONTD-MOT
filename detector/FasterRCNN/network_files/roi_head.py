...
class RoIHeads(torch.nn.Module):
    ...
    def postprocess_detections(self,
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            ...
            scores_index = scores.max(axis=1)
            boxes_select = boxes[torch.arange(boxes.shape[0]), scores_index.indices, :]
            labels_select = labels[torch.arange(labels.shape[0]), scores_index.indices]
            boxes, scores, labels = boxes_select, scores_index.values, labels_select
