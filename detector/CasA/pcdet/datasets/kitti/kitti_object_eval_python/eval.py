...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    Args:
        ...
        metric: eval type. 0: bbox, 1: bev, 2: 3d_det
...
def eval_class(gt_annos,
               ...
               compute_aos=False,
               num_parts=100):
    """Kitti eval. support 2d/bev/3d_det/aos eval. support 0.5:0.05:0.95 coco AP.
    Args:
        gt_annos: dict, must from get_label_annos() in kitti_common.py
        dt_annos: dict, must from get_label_annos() in kitti_common.py
...
def do_eval(gt_annos,
    ...
    if PR_detail_dict is not None:
        PR_detail_dict['3d_det'] = ret['precision']
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            result += print_str((f"bev  AP:{mAPbev[j, 0, i]:.4f}, "
                                 f"{mAPbev[j, 1, i]:.4f}, "
                                 f"{mAPbev[j, 2, i]:.4f}"))
            result += print_str((f"3d_det   AP:{mAP3d[j, 0, i]:.4f}, "
                                 f"{mAP3d[j, 1, i]:.4f}, "
                                 f"{mAP3d[j, 2, i]:.4f}"))
            ...
            result += print_str((f"3d_det   AP:{mAP3d_R40[j, 0, i]:.4f}, "
...
def get_coco_eval_result(gt_annos, dt_annos, current_classes):
    ...
    for j, curcls in enumerate(current_classes):
        ...
        result += print_str((f"bev  AP:{mAPbev[j, 0]:.2f}, "
                             f"{mAPbev[j, 1]:.2f}, "
                             f"{mAPbev[j, 2]:.2f}"))
        result += print_str((f"3d_det   AP:{mAP3d[j, 0]:.2f}, "
                             f"{mAP3d[j, 1]:.2f}, "
                             f"{mAP3d[j, 2]:.2f}"))
        if compute_aos:
