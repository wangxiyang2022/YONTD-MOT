...
def get_calib_from_file(filepath):
    ...
    with open(filepath) as f:
        for line in f.readlines():
            ...
            if line[:11] == "Tr_imu_velo":
                vti_mat = re.split(" ", line.strip())
                vti_mat = np.array(vti_mat[-12:], np.float32)

    data2["P2"]=P2.reshape(3, 4)
    data2["P3"]=P3.reshape(3, 4)
    data2["Tr_velo2cam"]=vtc_mat.reshape(3, 4)
    data2["R0"]=R0.reshape(3, 3)
    data2["Tr_imu2velo"] = vti_mat.reshape(3, 4)
...
class Calibration(object):
    ...
    def lidar_to_ref(self, pts_3d_lidar):
    ...
    def imu_to_rect(self, pts_imu):
        pts_velo = self.imu_to_lidar(pts_imu)
        pts_rect = self.lidar_to_rect(pts_velo)
        return pts_rect

    def imu_to_lidar(self, pts_3d_imu):
    ...
    def rect_to_imu(self, pts_rect):
        ...
        pts_imu  = self.lidar_to_imu(pts_velo)
        return pts_imu

    def lidar_to_imu(self, pts_3d_lidar):
        ''' Input: nx3 points in lidar coord.
            Output: nx3 points in IMU coord.
        '''
        pts_3d_lidar = self.cart2hom(pts_3d_lidar)  # nx4
        return np.dot(pts_3d_lidar, np.transpose(self.V2I))
